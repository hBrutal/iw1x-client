...
static LONG WINAPI CrashLogger(EXCEPTION_POINTERS* exceptionPointers)
{
    ...
    if (logFile.is_open())
    {
        ...
        DWORD exceptionAddr = (DWORD)(record->ExceptionAddress);
        DWORD exceptionCode = record->ExceptionCode;

        // Get module info for address
        IMAGEHLP_MODULE moduleInfo = { sizeof(moduleInfo) };
        if (SymGetModuleInfo(hProcess, exceptionAddr, &moduleInfo))
        {
            std::filesystem::path filePath = moduleInfo.LoadedImageName;
            std::string moduleName = filePath.filename().string();
            DWORD baseAddr = moduleInfo.BaseOfImage;
            DWORD offset = exceptionAddr - baseAddr;

            logFile << "Crash in: " << moduleName << " + 0x" << std::hex << offset << std::endl;
            logFile << "Module Base: 0x" << std::hex << baseAddr << std::endl;
        }
        else
        {
            logFile << "Module info not available." << std::endl;
        }

        logFile << "Exception Address: 0x" << std::hex << exceptionAddr << std::endl;
        logFile << "Exception Code: 0x" << std::hex << exceptionCode << std::endl;

        // Dump registers
        logFile << "\nRegisters:" << std::endl;
        logFile << "EAX: 0x" << std::hex << context->Eax << std::endl;
        logFile << "EBX: 0x" << std::hex << context->Ebx << std::endl;
        logFile << "ECX: 0x" << std::hex << context->Ecx << std::endl;
        logFile << "EDX: 0x" << std::hex << context->Edx << std::endl;
        logFile << "ESI: 0x" << std::hex << context->Esi << std::endl;
        logFile << "EDI: 0x" << std::hex << context->Edi << std::endl;
        logFile << "EBP: 0x" << std::hex << context->Ebp << std::endl;
        logFile << "ESP: 0x" << std::hex << context->Esp << std::endl;
        logFile << "EIP: 0x" << std::hex << context->Eip << std::endl;

        logFile << "\nA crash occured, please send your " + crashFilename + " file in the Discord server." << std::endl;
