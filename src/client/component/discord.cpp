...
namespace discord
{
		...
		while(true)
		{
			...
			if (connected)
			{
				...
				if (success)
				{
					...
					game::Cbuf_ExecuteText(game::EXEC_APPEND, command.c_str());
				}
				else
	...
	void updateInfo()
	{
		...
		if (*game::cls_state == game::CA_ACTIVE && !*game::clc_demoplaying)
		{
			char* cl_gameState_stringData = (char*)0x01436a7c;
			int* cl_gameState_stringOffsets = (int*)0x1434A7C;
			char* serverInfo = cl_gameState_stringData + cl_gameState_stringOffsets[game::CS_SERVERINFO];
			
			std::string g_gametype = game::Info_ValueForKey(serverInfo, "g_gametype");
			std::string mapname = game::Info_ValueForKey(serverInfo, "mapname");
			std::string sv_maxclients = game::Info_ValueForKey(serverInfo, "sv_maxclients");
			std::string sv_hostname = game::Info_ValueForKey(serverInfo, "sv_hostname");
			...
			*/
			std::string url = std::string("iw1x://") + game::NET_AdrToString(*game::clc_serverAddress);
			if (game::clc_serverAddress->type != game::NA_LOOPBACK)
			{
	...
	public:
		...
		{
			discord = game::Cvar_Get("discord", "0", game::CVAR_ARCHIVE);
