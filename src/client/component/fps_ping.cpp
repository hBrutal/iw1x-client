...
namespace fps_ping
{
	namespace
	{
		...
		void cg_draw_fps()
		{
			if (cg_drawFPS && cg_drawFPS->integer > 0)
			{
				...
				game::SCR_DrawString(x, y, font, scale, fps_color, fps_string, NULL, NULL, NULL);
		...
		void cg_draw_ping()
		{
			if (cg_drawPing->integer > 0 && game::CL_IsCgameInitialized())
			{
				/*auto* font = game::R_RegisterFont("fonts/consolefont");
				if (!font) return;*/
				int font = 1;
